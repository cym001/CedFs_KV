...
impl ActiveSequences {
    ...
    /// Return active hold counts for all blocks.
    pub fn all_block_hold_counts(&self) -> Vec<([u8; 32], usize)> {
        self.unique_blocks
            .iter()
            .filter_map(|entry| {
                let hold_count = entry.strong_count();
                if hold_count == 0 {
                    None
                } else {
                    Some((*entry.key(), hold_count))
                }
            })
            .collect()
    }

    /// Return hold-count statistics for all active blocks.
    ///
    /// # Returns
    /// - (total_entries, total_concurrency, max_concurrency)
    pub fn hold_statistics(&self) -> (usize, usize, usize) {
        let mut total_concurrency = 0usize;
        let mut max_concurrency = 0usize;
        let mut total_entries = 0usize;

        for (_, hold_count) in self.all_block_hold_counts() {
            total_entries += 1;
            total_concurrency += hold_count;
            max_concurrency = max_concurrency.max(hold_count);
        }
