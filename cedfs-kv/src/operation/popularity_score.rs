...
impl PopularityScoreOp {
    ...
    /// 域内迁移候选选择：选择并发度大于副本数*2的KV块进行域内副本扩展
    /// 
    /// 返回: Vec<(token_hash, offset, source_server, target_server)>
    /// 
    /// 选择逻辑：
    /// 1. 从并发度统计器中获取所有并发度 > 1 的块
    /// 2. 筛选出并发度 > 副本数 * 2 的 KvBlockMeta
    /// 3. 按并发度降序排序，选择前 k 个
    /// 4. source_server 和 target_server 都在 local_data_server_collect 中
    /// 5. source_server 有该 KvBlockMeta 副本
    /// 6. target_server 没有该副本
    /// 
    /// # 参数
    /// - `k`: 要选择的候选块数量
    /// 
    /// # 返回
    /// - Vec<(token_hash, offset, source_server, target_server)>: 迁移候选列表
    pub async fn intra_domain_transfer_candidate(&self, k: usize) -> Vec<([u8; 32], u32, DataServer, DataServer)> {
        use std::collections::BinaryHeap;
        use std::cmp::Reverse;
        ...
        for (token_hash, concurrency) in concurrency_data {
            ...
            // 从全局缓存表中获取块的元数据
            if let Some(kv_meta) = global_kvcache_table.get(&token_hash) {
                // 计算当前副本数（只统计在本地域内的副本数）
                let local_replica_count = kv_meta.server_id
                    .iter()
                    .filter(|id| local_server_ids.contains(id))
                    .count();
                
                // 如果副本数为0或已经所有本地服务器都有副本，则跳过
                if local_replica_count == 0 || local_replica_count >= local_data_servers.len() {
                    continue;
                }
                
                // 检查是否满足条件：并发度 > 副本数 * 2
                if concurrency > local_replica_count * 2 {
                    // 维护大小为 k 的最小堆
                    if heap.len() < k {
                        heap.push(Reverse((concurrency, token_hash)));
                    } else if let Some(&Reverse((min_concurrency, _))) = heap.peek() {
        ...
        // 从堆中提取结果并按并发度降序排序
        let mut candidates: Vec<(usize, [u8; 32])> = heap.into_iter()
            .map(|Reverse(tuple)| tuple)
            .collect();
        candidates.sort_by(|a, b| b.0.cmp(&a.0));
        
        // 为每个候选块选择 source_server 和 target_server
        let mut result = Vec::new();
        
        for (_concurrency, token_hash) in candidates {
            if let Some(kv_meta) = global_kvcache_table.get(&token_hash) {
                let offset = kv_meta.offset;
                
                // 找到本地域内有该副本的 server 作为 source_server
                let source_server = local_data_servers.iter()
                    .find(|ds| kv_meta.server_id.contains(&ds.id));
                
                // 找到本地域内没有该副本的 server 作为 target_server
                let target_server = local_data_servers.iter()
                    .find(|ds| !kv_meta.server_id.contains(&ds.id));
                
                if let (Some(source), Some(target)) = (source_server, target_server) {
                    result.push((
                        token_hash,
                        offset,
                        source.clone(),
                    ...
                    tracing::debug!(
                        "Intra-domain transfer candidate: token_hash {:?}, concurrency: {}, replicas: {}, source: {}, target: {}",
                        token_hash,
                        _concurrency,
                    ...
                    tracing::debug!(
                        "Cannot find suitable source or target server for token_hash {:?}",
                        token_hash
                    );
                }
            }
        ...
        tracing::info!(
            "Intra-domain transfer: Found {} candidates out of {} requested",
            result.len(),
            k
        );
